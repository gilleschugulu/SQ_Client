...
InterstageController = require 'controllers/ingame/interstage-controller'
...
module.exports = class GameController extends Controller
  ...
  # Workflow
  #  =>
  #  Call InterstageController with the first stage
  #  | Listen on event interstage:finish
  #  | Call Api to get all questions + bots
  ...
  finishGame: =>
    ...
    human = (player for player in @players when !(player instanceof Bot))[0]
    ...
    data =
      success    : !human.isEliminated()
      jackpot    : human.get('jackpot')
      good_answer: human.getGoodAnswerPercentage()
  ...
  flagQuestion: (params) =>
    console.log params
    questions = @game.stages[params.stageName].questions
    for question in questions
      if question.id is params.questionId
        question.used = yes
        break
    console.log @game

  # Load next interstage
  # --------------------
  loadNextInterstage: =>
    @currentStageIndex++
    ...
    @currentController = new InterstageController(@stages[@currentStageIndex].name, @currentStageIndex)
  ...
  initPlayers: =>
    @players = for bot in @game.bots
      new Bot(bot)
    @players.push new Player(mediator.user.getAttributes())
    # shuffle : to avoid teh player to be last
    # sort    : to put teh master in first position
    @players.shuffle().sort (p) -> !p.isMaster()
    console.log @players
